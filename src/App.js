...
const api_keys = [process.env.REACT_APP_GEMINI_API_KEY1, process.env.REACT_APP_GEMINI_API_KEY2]
...
const getOneTrialSummary = async (json_data, diagnosis, understanding, purpose, key) => {  
  ...
  async function run() {
    const model = genAI.getGenerativeModel({ model: "gemini-pro", safetySettings: safety_settings });
    const prompt = `${system_instr[understanding] + purpose_instr[purpose]} Summarize this JSON file 
                    of a clinical trial in less than 100 words with a Title, Date, Experiment, Results, 
                    and Limitations section for any helpful information on the study of ${diagnosis}. 
                    ...
                    are the keys and the content of each section are the values. Data to summarize: `;
    const result = await model.generateContent(prompt + JSON.stringify(json_data));
    const response = await result.response;
    const text = response.text();
    return text;
  ...
  try {
    ...
    console.error('Error in getOneTrialSummary():', error.message);
    return;
  }
}
...
function App() {
  ...
  const handleSubmit = (event) => {
    ...
    .then((user_condition) => {
      ...
      return getClinicalTrials(user_condition, process.env.REACT_APP_NUM_CLINICAL_TRIALS);
    })
    .then((response) => {
    ...
    .then(async (response) => {
        ...
        const trialSummaries = await Promise.all(response.map((trial_data, i) => {
            return getOneTrialSummary(trial_data, condition, knowledgeType, personType, api_keys[i % 2]);
        }));
        setTrialSummary(trialSummaries);
        // Get overall summary
  ...
  }, [trialSummary, summary, conditionInput]);
