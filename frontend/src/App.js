...
const getCombinedSummary = async (json_data, depth, understanding, purpose) => {
    const key = process.env.REACT_APP_GEMINI_API_KEY4;
    try {
        const response = await fetch('http://localhost:5000/get_overall_summary', {
            method: 'POST',
            headers: {
            ...
            mode: 'cors',
            body: JSON.stringify({
                json_data, depth, understanding, purpose, key
            })
        })
...
function App() {
  ...
  const handleSubmit = (event) => {
    ...
    .then((user_condition) => {
      ...
      if (condition === "Condition/disease Not Found.") {
        throw new Error(condition);
      }
      return getClinicalTrials(user_condition, 3 || process.env.REACT_APP_NUM_CLINICAL_TRIALS);
    })
    .then((response) => {
    ...
    .then(async (response) => {
        // Get overall summary
        const overall_summary = await getCombinedSummary(response, depthType, knowledgeType, personType);
        ...
        <div className="flex-box1">
            <div className="container">
                <form className="form-container" onSubmit={handleSubmit}>
                    ...
                    <option value="research">I am seeking additional information for my studies (Researcher)</option>
                    </select>
                    <br></br>
            ...
            {summary && summary.map((s, index) => (
                <div className="top-component" key={index}>
                    <h2 className="overall-title">Overall Summary</h2>
                    {s}
                </div>
            ))}
        </div>
